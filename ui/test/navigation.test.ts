...
import { EXTENSION_LABEL, GENERAL_CHECKS, HOME, PURGE_OUTDATED_DATA, STRUCTURAL_LINK } from '../src/navigation';

// The node ids are a contract with the Java side: XmlRepairNavigationExtender puts these exact strings into
// `?feature=` - HOME_FEATURE for the root node, GENERAL_CHECKS, STRUCTURAL_LINK and PURGE_OUTDATED_DATA for
// the XmlRepairNavigationNodes below it - and Home appends them to the portal's topic path. The Java tests pin the
...
describe('navigation node ids', () => {
  it('matches the ids the Java navigation nodes emit', () => {
    ...
    expect(STRUCTURAL_LINK).toBe('structural-link');
    expect(PURGE_OUTDATED_DATA).toBe('purge-outdated-data');
  });

  it('has a feature registered for every navigation node', () => {
    for (const id of [HOME, GENERAL_CHECKS, STRUCTURAL_LINK, PURGE_OUTDATED_DATA]) {
...
describe('findFeature', () => {
  ...
  it('names the navigation pages under the extension in the breadcrumb', () => {
    ...
    expect(findFeature(STRUCTURAL_LINK)?.breadcrumbTitle).toBe('Structural link');
    expect(findFeature(STRUCTURAL_LINK)?.breadcrumbParent).toBe(EXTENSION_LABEL);
